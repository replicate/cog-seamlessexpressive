...
MODEL_URL = "https://weights.replicate.delivery/default/SeamlessExpressive/seamless-expressive-cache.tar"
...
class Predictor(BasePredictor):
    def setup(self) -> None:
        if not os.path.exists("model"):
            download_weights(MODEL_URL, "model")
        self.device = (
        ...
        self.gcmvn_mean = torch.tensor(
            _gcmvn_mean, device=self.device, dtype=self.dtype
        )
        ...
        self.text_generation_opts = SequenceGeneratorOptions(
            beam_size=5, soft_max_seq_len=(1, 200), unk_penalty=torch.inf
        )
    ...
    def predict(
        ...
        audio_input: Path = Input(
            description="Input audio file to be translated and synthesized.",
        ),
        source_lang: str = Input(
            description="The original language of your input audio.",
            default="English",
            choices=LANG_TO_CODE.keys(),
        ),
        target_lang: str = Input(
            description="The target language for the output audio.",
            default="French",
            choices=LANG_TO_CODE.keys(),
        ),
        duration_factor: float = Input(
            description="Recommended: 1.0 for English, Chinese, Spanish; 1.1 for German, Italian; 1.2 for French.",
            default=1.0,
        ),
    ) -> Output:
        os.system(
            f"ffmpeg -i {audio_input} -acodec pcm_f32le -ar 16000 -ac 1 {audio_input.stem}.wav"
        )
        audio_input = Path(f"{audio_input.stem}.wav")

        src_lang = LANG_TO_CODE[source_lang]
        tgt_lang = LANG_TO_CODE[target_lang]
        print("Preparing to convert from", source_lang, "to", target_lang)

        fbank = self.collater(
            self.normalize_fbank(
                self.wav_to_fbank(self.decoder(self.mapper(audio_input)["data"]))
            )
        )

        output_path = Path("./tmp") / audio_input.stem
        ...
        with contextlib.ExitStack() as stack:
            ...
            valid_sequences: Optional[torch.Tensor] = None
            source = fbank["fbank"]
            ...
            if not valid_sequences.all():
                logger.warning(f"Sample has some corrupted input.")
            ...
            text = text_output[0]
            torchaudio.save(
                waveforms_dir / f"output.wav",
                speech_output.audio_wavs[0][0].to(torch.float32).cpu(),
                sample_rate=speech_output.sample_rate,
            )
            return Output(text_out=str(text), audio_out=waveforms_dir / f"output.wav")
